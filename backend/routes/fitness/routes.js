...
module.exports = function (app) {
  ...
  app.delete("/fitness/day/:dayId", verifyToken, async function (req, res) {
    try {
      await Day.deleteOne({ _id: req.params.dayId })
        .then(() => {
          res.status(201).send("Delete was successful!");
        })
        .catch((err) => {
  ...
  app.delete(
    ...
    async function (req, res) {
      try {
        await Day.deleteMany({ profileId: req.params.profileId })
          .then(async () => {
          ...
          .catch((err) => {
            res.send({
              err: err,
              msg: "Deleting all days for this profile went wrong!",
            });
          });
      } catch (error) {
        let errorObj = { body: req.body, errorMessage: "Server error!" };
